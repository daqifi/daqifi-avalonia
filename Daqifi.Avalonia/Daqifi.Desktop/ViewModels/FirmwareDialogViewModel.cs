...
public partial class FirmwareDialogViewModel : ObservableObject
{
    ...
    private async Task LoadFirmwareOptionsAsync()
    {
        try
        {
            ...
            {
                await Dispatcher.UIThread.InvokeAsync(AddOption);
            }
            else
            {
                AddOption();
    ...
    private async Task BrowseFirmwarePathAsync()
    {
        // Win32 OpenFileDialog → StorageProvider open picker (hal: file_pickers), owned by
        ...
        var owner = desktop.Windows.FirstOrDefault(w => w.DataContext == this) ?? desktop.MainWindow;
        if (owner == null)
        {
            return;
        }
        ...
        {
            AllowMultiple = false,
            FileTypeFilter = new[] { new FilePickerFileType("Firmware") { Patterns = new[] { "*.hex" } } }
        });
