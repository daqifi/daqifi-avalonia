...
public sealed class AppLoggerLoggerProvider : ILoggerProvider
{
    ...
    private sealed class AppLoggerBridge : ILogger
    {
        ...
        public AppLoggerBridge(IAppLogger appLogger, string category)
        {
            ...
            // device/environmental outcomes — a post-flash reconnect timeout, which is the PIC32's
            // JumpingToApp step (upstream issue #738) and the WiFi module's ReconnectingAfterFlash step
            // (upstream issue #776), where the firmware is already installed and a power-cycle finishes
            // the job.
            // Forwarding Core's duplicate Error to Sentry would both double-report real failures and undo
            // the coordinator's environmental downgrade. So route this category's Error/Critical to the
            // file only; the coordinator remains the single Sentry-capturing decision point for firmware.
