    ...
    public bool IsVisible
    {
        ...
        set
        {
            if (SetProperty(ref _isVisible, value) && ActualSeries != null)
            {
                ...
                // host there is no pumped UI loop; CheckAccess() is true on the calling thread, so
                // the work runs inline (the WPF Application.Current == null case).
                if (!Dispatcher.UIThread.CheckAccess())
                {
                    Dispatcher.UIThread.Invoke(ApplyVisibility);
                }
                else
                {
                    ApplyVisibility();
